...
global.authFile = `MiniBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MiniBot-MD', 'Edge', '2.0.0'] : methodCodeQR ? ['MiniBot-MD', 'Edge', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "MiniJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./MiniBotSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./MiniBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MiniBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./MiniJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./MiniJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./MiniJadiBot/${directorio}`).filter(fileInDir => {
...
const directories = ['./MiniBotSession/', './MiniJadiBot/']
